...
**On-device LLM is wired and is the default provider.** The protocol is [LLMService.swift](UnaMentis/Services/Protocols/LLMService.swift). Implementations exist for Anthropic, OpenAI, self-hosted, and a mock, plus [OnDeviceLLMService.swift](UnaMentis/Services/LLM/OnDeviceLLMService.swift). (Corrected 2026-08-12: this section originally said that service was marked incompatible and excluded in [project.yml](project.yml), and that on-device inference had not started. Both stopped being true on 2026-06-10. The service builds and is the default LLM provider.) The runtime is llama.cpp via the bundled `llama.xcframework`, not MLX-Swift, and it runs quantized GGUF models that [OnDeviceLLMModelManager.swift](UnaMentis/Services/LLM/OnDeviceLLMModelManager.swift) downloads for the device's RAM tier. There is a settings surface, [OnDeviceLLMSettingsView.swift](UnaMentis/UI/Settings/OnDeviceLLMSettingsView.swift), for model download. On-device TTS (Kyutai Pocket) is already integrated, and on-device STT (FluidAudio Parakeet) is gated behind a compile flag.
