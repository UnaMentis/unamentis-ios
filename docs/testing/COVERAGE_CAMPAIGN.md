...
| 2026-06-26 (wave 6) | 21.6% local | - | 20 | AI/models, cloud LLM streaming: OpenAILLMService 96.1%, AnthropicLLMService 97.2%, GoogleLLMService 96.7%. Built the URLProtocol test seam (reused the existing StubURLProtocol) and made the three providers' URLSession injectable (default .shared, behavior-preserving) so real request building + SSE/event-stream parsing + finish-reason mapping + the error-swallow contract are exercised. 38 real tests; consolidated a pre-existing 2-test Anthropic file (one hollow init assertion) into the comprehensive new suite |
...
The URLProtocol seam: providers that use `URLSession.shared` (OpenAI/Anthropic/Google/SelfHosted LLM) are made testable by injecting `session: URLSession = .shared` and driving an ephemeral session whose `protocolClasses` is the existing `StubURLProtocol`. This stubs ONLY the paid HTTP boundary (Real Over Mock compliant); all parsing under test is real. Providers that build their own `URLSession(configuration:)` (some STT/TTS) would each need the same injection before they can use this seam. (Denominator is the whole UnaMentis.app target, 114,732 executable lines including SwiftUI View bodies, so a fully-covered module moves the headline only a few tenths. Per-file coverage of the critical-path modules is the real signal.)
...
- **Wave 6 (DONE, green):** AI/models, cloud LLM streaming. Built the URLProtocol seam + injectable sessions; OpenAI/Anthropic/Google streaming services at ~96-97%.
- **Wave 7+ (prioritized order):** finish AI/models, the SelfHostedLLMService (streaming + the health/models/version endpoints, which use URLSession.shared so the seam already fits) and the STT/TTS providers (inject the session first where they build their own), then Embeddings. Then server comms (Core/Discovery, Core/Routing, Core/Config), then observability/stability (Core/Logging, Core/Telemetry, Core/Device), then the basics (Curriculum, ReadingList, Todo, KnowledgeBowl, etc.).
